...
const DoctorSelectPatientModal = (props) => {
    ...
    <Modal backdrop="static" keyboard={false} isOpen={props.openPopover} className="modal-dialog my-account-modal modal-dialog-centered hide-close" tabIndex="-1" autoFocus={false}>
        ...
        <ModalFooter className="justify-content-center">
                {!ShowPatientForm && <button onClick={()=>props.setPopover(!props.openPopover)} type="button" className="btn brand-secondary rounded-pill px-5">Cancel</button> }
                {ShowPatientForm && <button onClick={()=>setShowPatientForm(!ShowPatientForm)} type="button" className="btn brand-secondary rounded-pill px-5">Back</button>}
                <button type="button" onClick={()=>props.setPopover(!props.openPopover)} className="btn btn-brand-gradient rounded-pill px-5 ml-3">Save &amp; Continue</button>
        </ModalFooter>
    </Modal>)
