...
const MemberShipProductDiscountInfo = (props) => {
        ...
        <React.Fragment>
                ...
                { validate.isEmpty(props.isDiscontinued) &&
                    <>
                            ...
                            { props.product.isGeneral === 'N' && validate.isNotEmpty(props.product.packSize) && props.product.packSize > 0 && validate.isNotEmpty(props.product.displayPrice) && props.product.displayPrice > 0 && <p className="mb-0 text-secondary font-14">{props.product.packSize} Unit{props.product.packSize > 1 && "s"}/pack</p> }
                                <React.Fragment>
                                    { validate.isNotEmpty(props.product.displayPrice) && props.product.displayPrice > 0 && <React.Fragment>
                                        <h4 className={`mb-0 ${props.pharmaSubscribed ? 'strikethrough-diagonal' : ''}`}>MRP
                        ...
                        {validate.isEmpty(props.memberShipPrice) && validate.isNotEmpty(props.discountString) && <div className="disc-offer-badge d-inline-block">
                            <p className="mb-0 font-weight-bolder">{props.discountString}
                                <GetCurrentMaxDiscount />
                            </p>
                        </div> }
                        <div className="mt-3 mb-4">
                            {validate.isNotEmpty(props.memberShipPrice) && <><div className="align-items-center d-flex justify-content-end flex-wrap">
                                <span className="font-14">Member Price</span>
                                <h5 className="font-weight-bolder mb-0">&nbsp;   <small className="rupee ml-1">₹</small>{props.memberShipPrice}</h5>
                            </div>
                            {validate.isNotEmpty(props.product) && props.product.isGeneral === "N" && validate.isNotEmpty(props.product.packSize) && props.product.packSize > 1 && <span class="float-right font-12 mb-2">
                                (
                                    <small className="rupee">&#x20B9;</small>
                                    {(props.membershipPricePerUnit)} per unit
                                )
                            </span>}
                            </>}
                            {!props.pharmaSubscribed && validate.isNotEmpty(props.bestPharmaPlanInfo) && 
                            <button title="MedPlus Advantage Member Price" className={`${window.screen.width > 1368 ? "custom-btn-xl":"custom-btn-lg font-12"} brand-secondary btn btn-block  px-4 rounded-pill`} type="button" role="button" onClick={() => redirectToPlanDetail(props.bestPharmaPlanInfo.planName,props.bestPharmaPlanInfo.planId)}>
                                <span>Become a member for
                                    <small className="ml-1">₹</small>{parseFloat(props.bestPharmaPlanInfo.price)}/-</span>
                            </button>}
                        </div>
                        ...
                        :
                            <React.Fragment>
                                {!isDrugScheduleX && (props.product.isGeneral === "Y" || props.isFridgeItemAllowed) && (validate.isEmpty(props?.replacementProduct?.name) || (validate.isNotEmpty(props?.replacementProduct?.name) && props.availQty > 0)) && <React.Fragment>
                                    {validate.isNotEmpty(props.product.productId) && <div className='w-100'>
                        ...
                        {validate.isNotEmpty(props.memberShipPrice) && validate.isNotEmpty(props.discountString) && <p className="font-14 mb-0 mt-2 text-lg-right text-secondary">{props.discountString}<GetCurrentMaxDiscount/> </p>}
                    </>
                }
        </React.Fragment>
    );
}
export default MemberShipProductDiscountInfo