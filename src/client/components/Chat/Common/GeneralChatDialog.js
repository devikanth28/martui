...
import MedplusChatBotLogo from '../images/medplus-chat-bot-logo.png';
...
const GenaralChatDialog = (props) => {
  ...
  const [minInputLength, setMinInputLength] = useState(defaultMinInputLength);
  ...
  const pincodePattern = new RegExp(/^[0-9]{0,6}$/);
  ...
  const renderHistory = async (shouldScrollBottom, getHistoryCount, previousDateMessage) => {
    let historyObject = null;
    if (extras && extras.loadHistory) {
      const loadBy = extras.loadHistory.loadBy;
      if (loadBy === "customerId") {
          historyObject = await loadHistory(
            extras.customerId,
            loadBy,
            getHistoryCount,
            CONFIG.API.BOT.GET_BOT_CUSTOMER_HISTORY.PATH
          );
      }
    }
    if (historyObject && historyObject.history && historyObject.history.length > 0) {
      ...
      setConversation((conversation) => {
        let lastHistoryMessage = history[history.length - 1];
        lastHistoryMessage = history[history.length - 1];
        const lastHistoryMessageDate = lastHistoryMessage.date.toLocaleDateString(
          { weekday: "long", year: "numeric", month: "long", day: "numeric" }
        );
        if ( conversation.length > 0 && previousDateMessage.date !== lastHistoryMessageDate) {
  ...
  const loadHistory = async (id, loadBy, getHistoryCount, path) => {
    if (!id || !loadBy) return;
    let params = {
    ...
    try {
      ...
      if (message && message.blocks) {
        ...
        setPincodeValidation(
          message.blocks.pincodeValidation
            ? message.blocks.pincodeValidation
            : false
        );
      }
  ...
  const sendTextMessage = async (event) => {
    if (event) event.preventDefault();
    if(pincodeValidation && !pincodePattern.test(userInputRef.current.value)){
  ...
  const onDescriptionChange = (e) => {
    if (pincodeValidation && !pincodePattern.test(e.target.value)) {
      if (description && description.length > 0) setDescription(description);
      else setDescription("");
      return;
    }
    ...
    if (pincodeValidation && (e.target.value.trim().length < 6 || e.target.value.trim() < 100000)) {
      setDisableSubmitButton(true);
      ...
      <div id="message_window">
        <div className="powered_by">
          ...
          >
            ...
            <div>
            <p className="mb-0 ml-2"><strong>MedPlus</strong>Chat bot</p>
            { botHeaderStatus && (botHeaderStatus === "Online" || botHeaderStatus === "connecting...")  &&
              <small class="help-text d-flex align-items-center">
          ...
          <button onClick={()=>closeChatBot()} class="end-chat-btn btn btn-sm">
              End Chat
          </button>
        </div>
        <section
        ...
        {errorMessage && <div class="error-message p-2">{errorMessage}</div>}
        {enableUserInput && (
          <footer>
            <form onSubmit={sendTextMessage}>
              ...
              />
              <button type="submit" className="btn btn-danger" disabled={disableSubmitButton} style={{"min-height":"unset"}}>Send</button>
            </form>
          </footer>
        )}
      </div>
...
export default GenaralChatDialog;
