...
const OnlyHomeSamplePickup = (props) => {
    ...
    const addOrUpdateAddressInfo = (addressInfo) => {
        ...
        if(addEditAddress === "EDIT_ADDRESS") {
            ...
            addressInfo.addressLine2 = selectedLocality.locationName;
        ...
        <section className="body-height">
            ...
            {validate.isNotEmpty(customerAddressList) && customerAddressList.length > 0 && validate.isEmpty(addEditAddress) && 
                <div className="select-address px-3 py-2">
                    <p className="title d-flex" style={{'justify-content':'normal'}}><span className="mr-5">Saved Address: </span>{!props.isNewAddressAdded && <button type="submit" role="button" className="btn btn-brand ml-5" onClick={() =>openNewAddressSection()}>ADD NEW ADDRESS</button>}</p>
                            ...
                            <form action="" autoComplete="off">
                                ...
                                <div className="d-flex justify-content-betwween mt-3">
                                    <div className="w-50 mr-3"> 
                                        <button type="button" role="button" className="btn btn-block brand-secondary px-5" onClick={() =>cancelButtonAction()}>CANCEL</button>
                                    </div>
                                    <div className="w-50"> 
                                        <button type="button" role="button" className="btn btn-block btn-brand-gradient px-5" onClick={() =>addOrUpdateAddressInfo(customerAddress)}>{addEditAddress === 'ADD_ADDRESS' ? 'Save' : 'Update'}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
            ...
            {validate.isEmpty(customerAddressList) && validate.isEmpty(addEditAddress) &&
                <div className="col text-center p-3 ">
                    ...
                    <button type="submit" role="button" className="btn btn-brand-gradient rounded-pill my-2 px-4 custom-btn-lg" onClick={() =>openNewAddressSection()}>Add New Address</button>
                </div>
            }
        </section>
    );
