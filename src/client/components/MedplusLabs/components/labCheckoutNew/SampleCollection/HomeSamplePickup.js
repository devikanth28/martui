            ...
            <section className="body-height">
                ...
                <div className="home-delivery-option-tab">
                    <div className={validate.isEmpty(addEditAddress) ? 'select-address' : ''}>
                        {validate.isEmpty(addEditAddress) && validate.isNotEmpty(customerAddressList) && <p className="title">Saved Address: {!props.isNewAddressAdded && <button type="submit" role="button" className="btn btn-brand-gradient rounded-pill" onClick={() =>openNewAddressSection()}>add new address</button>}</p>}
                        ...
                        {validate.isNotEmpty(addEditAddress) && <AddNewAddress selectedLocality={selectedLocality} addEditAddress={addEditAddress} editAddressInfo={editAddressInfo} cancelButtonAction={cancelButtonAction} setIsNewAddressAddedFlag={props.setIsNewAddressAddedFlag} showLoader={props.showLoader} addOrUpdateDeliveryDetails={addOrUpdateDeliveryDetails}/>}
                    </div>
                    {validate.isEmpty(customerAddressList) && validate.isEmpty(addEditAddress) && 
                        <div className="col text-center p-3 ">
                            ...
                            <button type="submit" role="button" className="btn btn-brand-gradient rounded-pill my-2 px-4" onClick={() =>openNewAddressSection()}>add new address</button>
                        </div>
                    }
                </div>
            </section>
