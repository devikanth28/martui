...
const ReportDeliveryAddress = (props) => {
    const address = props.shoppingCart.address
    return (
        <section className="delivery-detail mb-3">
            {address && <div className="header">
                <p>Report Delivery Address</p>
            </div>}
            {address && <div className="body">
                <p className="font-weight-bold">{address.firstName}</p>
                <address className="no-select p-0">
                    <small className="d-flex">{address.addressLine1}, {address.addressLine2}  </small>
                    <p className="d-block mt-3">
                        ...
                        <a className="text-primary" href="tel:9666667691" title="Click to Call">{address.mobileNo}</a>
                        {/* <a className="text-primary ml-3" href="http://maps.google.com/?saddr=17.44841390000065,78.3908438000143&amp;daddr=17.44126000,78.39124000" target="_blank" title="Get Directions">
...
export default connect(mapStateToProps)(ReportDeliveryAddress);