...
import Alert ,{ALERT_TYPE_ERROR,ALERT_TYPE_SUCCESS,ALERT_TYPE_INFO} from '../../Common/Alert';
...
const EditPrescription = (props) => {
    ...
    useEffect(() => {
        if(validate.isNotEmpty(uploadedFilesInfo)) {
            let imageThumbnailURLs = {};
            uploadedFilesInfo.forEach((eachFileInfo, index) => {
    ...
    const closeBtn = <button type="button" className="close" aria-label="Close" onClick={()=> {setUploadedFilesInfo([]); props.closeModal();}} disabled={(isProceedLoading || isUploadInProgress)}>
    ...
    const validateFilesUpload = (noOfNewFiles, maxSizeOfNewFile) => {
        ...
        let noOfFiles = 0;
        if(validate.isNotEmpty(props.uploadedImages)){
            noOfFiles = props.uploadedImages.length;
        }
        if(validate.isNotEmpty(uploadedFilesInfo)){
            noOfFiles += uploadedFilesInfo.length;
        }
        const noOfUploadedFiles = (noOfFiles + noOfNewFiles);
        if(noOfUploadedFiles > 8) {
            const remainNoOfFiles = (8 - noOfFiles);
    ...
    const editPrescription = () => {
        if(validate.isEmpty(uploadedFilesInfo)) {
            setAlertInfo({ message: "Upload prescription to submit", type:ALERT_TYPE_INFO });
            return;
        }
        setProceedLoading(true);
        prescriptionService.editPrescription(props.prescriptionOrderId, uploadedFilesInfo).then(response => {
            setProceedLoading(false);
            if (validate.isNotEmpty(response) && validate.isNotEmpty(response.statusCode) && "SUCCESS" == response.statusCode) {
                props.updateNewImages(response.dataObject.prescriptionOrder);
    ...
    return(
        <React.Fragment>
            ...
            <Modal backdrop="static" keyboard={false} isOpen={props.modal} className="modal-lg modal-dialog-centered my-account-modal edit-prescription-popup" tabIndex="-1">
                <ModalHeader close={closeBtn}>Edit Prescription</ModalHeader>
                    <ModalBody>
                    <div className="upload-container">
                    {validate.isNotEmpty(props.uploadedImages) && props.uploadedImages.map((prescriptionImg,index)=>{
                     return (
                        <div className="each-upload"  key={"uploaded-"+index}>
                            <div className="content">
                                <img id={"uploaded-"+index} src={validate.isNotEmpty(prescriptionImg.thumbnailPath) ? prescriptionImg.thumbnailPath : DefaultBanner} title={prescriptionImg.originalImageName} height="100"/>
                                <p className="text-truncate" title={prescriptionImg.originalImageName}>{prescriptionImg.originalImageName}</p>
                            </div>
                        </div>
                     )})}
                    ...
                    <div className="upload-container">
                        {validate.isNotEmpty(uploadedFilesInfo) && uploadedFilesInfo.map((eachFileInfo, index) => {
                        return (
                        ...
                        <br/>
                        <p className="mb-0 small">Select file to upload <sup className="text-brand">*</sup><span className="text-muted ml-2">Supported Files: jpeg/jpg/png/gif/pdf, Size upto 4MB</span></p>
                        ...
                        <div className="text-center mt-4">
                            <button type="button" className="btn  brand-secondary px-5 rounded-pill custom-btn-lg" onClick={()=> {setUploadedFilesInfo([]); props.closeModal();}} disabled={(isProceedLoading || isUploadInProgress)}>Cancel</button>
                            <button type="button" className="btn btn-brand-gradient ml-3 px-5 rounded-pill custom-btn-lg" disabled={isUploadInProgress} onClick={()=>{editPrescription()}}>
                            {isProceedLoading ? "" :"Update"}
                            {isProceedLoading &&
                                <React.Fragment>
