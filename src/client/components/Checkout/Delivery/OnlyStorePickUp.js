        ...
        <section className="body-height">
            <div className="header">
                <p>
                    ...
                    {props.checkoutVerticle == "LAB_CHECKOUT" && "Give Sample at Collection Centre"}
                </p>
            </div>
            {validate.isNotEmpty(prevSelectedStoreInfo) && <div className="selectpickupstore">
                <p className=" px-3 mb-0 badge-title success">
                ...
                <address className={props.selectedDeliveryType.STORE_PICK_UP === prevSelectedStoreInfo.storeId ? "no-style active" : "no-style"} onClick={e => props.setPickUpStoreInfo(prevSelectedStoreInfo.storeId)}>
                    <p className="title">
                    ...
                    <p className="title">
                        ...
                        <a className="text-primary" aria-label="click to Call" role="link" href={"tel:"+prevSelectedStoreInfo.phoneNumber} title="Click to Call"> {prevSelectedStoreInfo.phoneNumber}</a>
                        </span>
                        {/* <span className="mr-3 small">
                        ...
                        <a className="mr-3 small text-primary" aria-label="Get Direction" role="link" rel="noopener" href={getDirection} target="_blank" title="Get Directions">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18">
                                <defs>
                                    <style>{defsCss}</style>
                                </defs>
            ...
            {validate.isNotEmpty(prevSelectedStoreInfo) && validate.isNotEmpty(nearByStores) && Object.keys(nearByStores).length > 0 && <hr className="solid"/>}
            {nearByStores && props.checkoutVerticle != 'LAB_CHECKOUT' && Object.keys(nearByStores).length > 0 && <div className="nearbystore">
                ...
                <div className="address-container">
                    {nearByStores && Object.keys(nearByStores).map(function (key) {
                        const isLocationMatched = validate.isEmpty(selectedLocation) || (validate.isNotEmpty(selectedLocation) && selectedLocation === 'ALL') || (validate.isNotEmpty(selectedLocation) && (nearByStores[key].address.toLowerCase().includes(selectedLocation.toLowerCase()) || nearByStores[key].locality.toUpperCase().replace(/ /g, "_") === selectedLocation || nearByStores[key].storeName.toLowerCase().includes(selectedLocation.toLowerCase())));
                        ...
                        return (isLocationMatched && isSearchKeyMatched && <PickUpStore key={key} eachStoreInfo={nearByStores[key]} setPickUpStoreInfo={props.setPickUpStoreInfo} selectedDeliveryType={props.selectedDeliveryType} deliveryType={props.deliveryType} locationLatLong={props.selectedLocality.locationLatLong} />)
                    })}
                </div>
            </div>
            }
            {props.checkoutVerticle == 'LAB_CHECKOUT' && (nearByStores || otherNearByStores) && <div className="nearbystore">
                ...
                <div className="mt-4">
                    <SearchBar selectedLocation={selectedLocation} setSelectedLocation={(location) => setSelectedLocation(location)} nearByStores={nearByStores} otherNearByStores={otherNearByStores} searchText={searchText} setSearchText={(text) => filterStores(text)} {...props} />
                </div>                
                {filteredLabStores && Object.keys(filteredLabStores).length > 0 && <React.Fragment>
                    <p className="title mx-3 mb-0">Centers Available For All Selected Tests</p>
                    <div className="address-container">
...
export default OnlyStorePickUp;
                    ...
                    <div className="col-4">
                        <form className="locality">
                            <div className="filled-form form-group">
                                ...
                                {validate.isEmpty(props.searchText) && <button aria-label="search-icon" role="button" className="btn btn-link shadow-none icons search-icn" title=""></button>}
                                {validate.isNotEmpty(props.searchText) && <button aria-label="clear-icon" role="button" className="btn btn-link shadow-none icons clear-icn d-block" title="" onClick={() => props.setSearchText('')}></button>}
