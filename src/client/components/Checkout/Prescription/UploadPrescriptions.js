...
const UploadPrescriptions = (props) => {
    ...
    useEffect(() => {
        if(validate.isNotEmpty(props.uploadedFilesInfo)) {
            let imageThumbnailURLs = {};
            let imagesForZoom = [];
            props.uploadedFilesInfo.forEach((eachFileInfo, index) => {
                imageThumbnailURLs[index] = (props.imageServerUrl+"/"+eachFileInfo.thumbnailPath);
                imagesForZoom.push(props.imageServerUrl+"/"+eachFileInfo.imagePath);
            });
            setImageThumbnailPaths(imageThumbnailURLs);
            setTempImageThumbnailPaths(imageThumbnailURLs);
            setImagesForZoom(imagesForZoom);
        }
    }, [props.uploadedFilesInfo]);
    ...
    const uploadMorePrescriptions = (event) => {
        if(props.isFilesUploadInProgress) {
    ...
    return (
        <React.Fragment>
            ...
            {/* Upload prescription section starts */}
            <div className="header"><p>Upload Prescriptions</p></div>
            <div className="upload-prescription">
                <a className="back-btn" href="javascript:void(0)" title="Back to options" onClick={() => props.toggleUploadPrescriptions(false)}>
                 ...
                 <div className="row mx-0">
                    <div className="col-6 p-0">
                        {validate.isNotEmpty(props.uploadedFilesInfo) &&
                            <div className="uploaded-prescriptions">
                                {props.uploadedFilesInfo.map((eachFileInfo, index) => {
                                    return(eachFileInfo && 
                                        <React.Fragment key={index}>
                                            <div className="each-prescription">
                                                <img id={"prescriptionImage-"+ index} src={validate.isNotEmpty(imageThumbnailPaths) ? imageThumbnailPaths[index] : DefaultBanner} alt="prescription image" title="prescription image" height="100" onError={() => handleImageLoadError(index)} onClick={() => openLightBox(index)}/>
                                                {!props.isFilesUploadInProgress &&
                                                    <span className="delete-prescription" title="Delete prescription" onClick={() => props.removeUploadedFileInfo(index, eachFileInfo.originalImageName)} >
                            ...
                            {validate.isNotEmpty(imagesForZoom) && <HealthRecordZoom imageIndex={activeIndex} isLightBoxOpen={isLightBoxOpen} closeLightBox={closeLightBox} images={imagesForZoom} reactModalStyle={{"z-index":1060}}/>}
                            </div>
                        }
                        {validate.isEmpty(props.uploadedFilesInfo) && <p className="no-prescription-text">No Prescription Uploaded</p>}
                    </div>
                    <div className="col-6">
                        <div className="additional-notes">
                            ...
                            <React.Fragment>
                                <a onClick={() => setShowSampleImage(true)} href="javascript:void(0)" title="View sample prescription" class="sample-prescription">View sample prescription</a>
                                {showSampleImage &&
                                    <ImageLightBox style={{ "z-index": 1060 }}
                                        prescImages={imagesForZoomSample}
                                        imageIndex={0}
                                        mainSrc={SampleImage}
                                        hideDownload={true}
                                        onCloseRequest={() => setShowSampleImage(false)}
                                    />}
                            </React.Fragment>
                        </div>
                        <div className="file-upload-container">
                            {(validate.isEmpty(props.filesToUpload) || (props.filesToUpload.length < 4)) &&
                                <React.Fragment>
                                    <h6>Select file to upload<span className="text-brand ml-1">*</span></h6>
                                    <input type="file" id="fileUpload" name="fileUpload" accept="image/*,application/pdf" ref={addMoreFiles} multiple onChange={event => props.uploadFiles(event)} hidden />
                                    <div className="add-more-div mt-0" onClick={event => uploadMorePrescriptions(event)}>
                            ...
                            <p className="small">Supported Files: jpeg/jpg/png/gif/pdf, Size upto 4MB</p>
                            {validate.isNotEmpty(props.filesUploadProgress) && validate.isNotEmpty(props.filesUploadProgress.percentCompleted) && (parseInt(props.filesUploadProgress.percentCompleted) >= 0) && (parseInt(props.filesUploadProgress.percentCompleted) < 100) &&
                                <div className="file-upload-progress-container">
                                    <div className="file-upload-details">
                                        ...
                                        {/* <a className="icons clear-icn" title=""></a> */}
                                        <div className="file-upload-progress">
                                            <small>{props.filesUploadProgress.noOfFiles + " file(s) uploading..."}</small>
                                            <Progress barClassName="bar" color="success" value={props.filesUploadProgress.percentCompleted} ><span className="sr-only percent">0% Complete</span></Progress>
                                            {/* <small className="float-left">{"3.5 MB upto 4 MB--"+ props.filesUploadProgress.fileSize}</small> */}
                                            <small className="file-uploading-text">Uploading... <span className="text-dark ml-1 percent">{props.filesUploadProgress.percentCompleted + "%"}</span></small>
                                        </div>
                                    </div>
                                </div>
                            }
            ...
            {/* Upload precription section ends */}
        </React.Fragment>
    );
}
