...
import { KEYS, MIN_ZOOM_LEVEL, MAX_ZOOM_LEVEL, ZOOM_RATIO, WHEEL_MOVE_X_THRESHOLD, WHEEL_MOVE_Y_THRESHOLD, ZOOM_BUTTON_INCREMENT_SIZE, ACTION_NONE, ACTION_MOVE, ACTION_SWIPE, ACTION_PINCH, SOURCE_ANY, SOURCE_MOUSE, SOURCE_TOUCH, SOURCE_POINTER, MIN_SWIPE_DISTANCE, } from './LightBoxConstant';
...
class ReactImageLightbox extends Component {
  ...
  getBestImageForType(srcType) {
    ...
    } else if(imageSrc && imageSrc.endsWith(".pdf")) { 
      return {
        src: imageSrc,
        height: 700,
        width: 500,
  ...
  requestMove(direction, event) {
    ...
    const nextState = { zoomLevel: MIN_ZOOM_LEVEL, offsetX: 0, offsetY: 0 };
  ...
  render() {
    ...
    // Main Image
    addImage('mainSrc', 'ril-image-current', { x: -1 * offsetX, y: -1 * offsetY, zoom: zoomMultiplier });
    // Previous Image (displayed on the left)
    addImage('prevSrc', 'ril-image-prev ril__imagePrev', { x: -1 * boxSize.width });

    const modalStyle = {
      overlay: { zIndex: 1000, backgroundColor: 'transparent', ...reactModalStyle.overlay },
      content: {
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Image holder */}
          <div className="ril-inner ril__inner" onClick={clickOutsideToClose ? this.closeIfClickInner : undefined} >{images}</div>
            ...
            <ul className="ril-toolbar-left ril__toolbarSide ril__toolbarLeftSide">
              <li className="ril-toolbar__item ril__toolbarItem">
                ...
                <React.Fragment>
                  ...
                  {this.props.hideDownload ? "" :  <a className="btn btn-link ml-2" target="_blank" title="Download Image" href={prescImages[imageIndex]}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24.001">
                      <g transform="translate(-180.258 -387.452)">
                        <rect fill="none" width="20" height="20" transform="translate(180.258 387.452)"></rect>
                        <path fill="#ffffff" d="M-4848.307-664.18a.752.752,0,0,1-.752-.752.752.752,0,0,1,.752-.752h19.828a.752.752,0,0,1,.751.752.752.752,0,0,1-.751.752Zm9.862-4.456-.239-.049a.72.72,0,0,1-.241-.162l-4.874-4.873a.751.751,0,0,1,0-1.062.749.749,0,0,1,1.063,0l3.591,3.59V-687.43a.751.751,0,0,1,.751-.751.751.751,0,0,1,.752.751v16.238l3.591-3.59a.749.749,0,0,1,1.063,0,.752.752,0,0,1,0,1.061l-4.874,4.874a.726.726,0,0,1-.242.162l-.236.049-.054.01Z" transform="translate(5030.652 1075.633)"></path>
                      </g>
                    </svg>
                  </a>}
                </React.Fragment>
