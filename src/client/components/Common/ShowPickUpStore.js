...
const ShowPickUpStore = (props) => {
    const selectedLocality = getSelectedLocality();
    let getDirectionLink = (props.locationLatLong && selectedLocality) ? "http://maps.google.com/?saddr="+selectedLocality.locationLatLong+"&daddr="+props.locationLatLong : "";
    return(
        <React.Fragment>
            <div className={`${props.excludeBodyClass?'':'body'}`}>
                {props.isLab && <small class="mb-2 d-block text-secondary mt-2">Address</small>}
                <p className={`${props.isLab?'font-weight-bold mb-1': (`${props.isFromDoctorPage}`? ' font-weight-bold mb-2':'font-weight-bold')}`}>{props.pickStoreName}</p>
                {!props.isFromDoctorPage &&
                    <address className="no-select p-0 rounded-0">
                        {props.isSmallAddressRequired &&
                            <small className="d-flex">
                                {props.pickUpAddress}
                            </small>
                        }
                        {!props.isSmallAddressRequired && props.pickUpAddress}
                        <p className="d-block mt-3">
                            {props.phoneNumber &&
                            <React.Fragment>
                                <a className="text-primary btn btn-sm btn-link ml-n2" aria-label="click to Call" role="link" href={`tel:${getCountryCodeForMobile(props.phoneNumber)+props.phoneNumber}`} title="Click to Call">
                            ...
                            <a rel="noopener" className="text-primary ml-3 btn btn-sm btn-link" href={getDirectionLink} target="_blank" aria-label='Get Directions' role="link" title="Get Directions">
...
export default ShowPickUpStore;