...
import EditPatientForm from '../DoctorConsultation/EditPatientForm'

const DoctorShoppingCartNew = (props) => {
    ...
    <Modal backdrop="static" keyboard={false} isOpen={props.openPopover} className="modal-dialog my-account-modal modal-dialog-centered hide-close" tabIndex="-1" autoFocus={false}>
        ...
        <ModalFooter className="justify-content-center">
                {!ShowPatientForm && <button onClick={()=>props.setPopover(!props.openPopover)} type="button" className="btn brand-secondary px-5 rounded-pill">Cancel</button> }
                {ShowPatientForm && <button onClick={()=>setShowPatientForm(!ShowPatientForm)} type="button" className="btn brand-secondary px-5 rounded-pill">Back</button>}
                <button type="button" onClick={()=>props.setPopover(!props.openPopover)} className="btn btn-brand-gradient ml-3 px-5 rounded-pill">Save & Continue</button>
        </ModalFooter>
    </Modal>)

}

export default DoctorShoppingCartNew